...
- 🤖 **AI-ассистент** - ответы через Gemini/OpenRouter/Airforce/Pollinations
...
3. Получи API ключи:
   ...
   - [Airforce API](https://api.airforce) (опционально, бесплатно)
...
- `AIRFORCE_API_KEY_1`, `AIRFORCE_API_KEY_2`, ... - для Airforce API (бесплатно, с автоматическим переключением при ошибках)
- `AIRFORCE_MODELS` - модели Airforce (по умолчанию: `claude-sonnet-4.6`)
- `POLLINATIONS_API_KEY` - для приватных запросов
- `LLM_PROVIDER_ORDER` - порядок провайдеров (по умолчанию: `gemini,openrouter,airforce,pollinations`)
...
- `/settings` - настройки
- `/help` - все команды
- `/provider` - выбор провайдера LLM
- `/or_model` - модель OpenRouter
- `/poll_text` - модель Pollinations
- `/msgsize` - размер ответа
- `/interval` - интервал автопостов
- `/minmsgs` - мин. сообщений для автопоста
- `/draw_model` - модель генерации изображений
- `/modval` - настройки модерации
- `/logchannel` - канал логирования
- `/export` - экспорт настроек
- `/import` - импорт настроек
- `/addf` - добавить в фильтр слов
- `/remf` - убрать из фильтра слов
- `/fs` - список фильтров слов
...
MIT License - используй на свой страх и риск
